...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module(f"Niskala.modules.{module_name}")
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_text(
            f"Hai Bro I'm Niskala!\n<b>Active since:</b> <code>{uptime}</code>",
...
def help_button(update, context):
    ...
    try:
        if mod_match:
            ...
            text = f"`Hᴇʀᴇ Iꜱ Tʜᴇ Hᴇʟᴘ`「*{HELPABLE[module].__mod_name__}*」 `Mᴏᴅᴜʟᴇ:`\n{HELPABLE[module].__help__}"
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 1338853808 and DONATION_LINK:
            update.effective_message.reply_text(
